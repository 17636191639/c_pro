...
void merge_sort2(int List[], int *v_aux, int length)
{
    int size = 1;
    int low;
    int mid;
...
{
	int v[24] = {14, 99, 65, 97, 73, 13, 18, 0, 55, 32, -11, 101, 999, -111, 11, -90, 87, -60, 81, -10, 12, -119, 911, 375};
	int a[24];
	...
	memcpy(a, v, sizeof(a));
	print_array(a, len);
	merge_sort(a, v_aux, 0, len - 1);
	print_array(a, len);

	memcpy(a, v, sizeof(a));
	print_array(a, len);
	merge_sort2(a, v_aux, len);
	print_array(a, len);
